...
describe('The computeTextAlternative function', () => {
    ...
    it('returns correct computation details for simple button input', () => {
        lit_1.render(lit_1.html `<button id="test">Hello world</button>`, container);
        const elem = document.getElementById('test');
        expect(compute_text_alternative_1.computeTextAlternative(elem)).toEqual({
            name: 'Hello world',
            steps: [
    ...
    it('returns correct computation details for aria-labelledby references', () => {
      ...
      `, container);
        ...
        expect(compute_text_alternative_1.computeTextAlternative(elem1)).toEqual({
            name: 'Hello world',
            steps: [
    ...
    it('can handle elements in iframes', async () => {
        lit_1.render(lit_1.html `<iframe srcdoc="<body></body>"></iframe>`, container);
        const iframe = container.querySelector('iframe');
        await iframeLoadedPromise(iframe);
        const iframeDocument = iframe.contentWindow.document;
        lit_1.render(lit_1.html `<button>Inside iframe</button>`, iframeDocument.body);
    ...
    it('can handle Unicode BiDi control characters', () => {
        ...
        for (const elem of targets) {
            expect(elem).toHaveTextAlernative('يلا\u202C foo \u202Aيلا bar \u202Bيلا');
        }
...
async function iframeLoadedPromise(iframe) {
    return new Promise((resolve) => {
        iframe.addEventListener('load', resolve, { once: true });
    });
}
//# sourceMappingURL=compute_text_alternative_test.js.map